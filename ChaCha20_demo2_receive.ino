...
void processData(String data, uint32_t uart_time)
{
  ...
  uint32_t t1=micros();
  chacha.decrypt(decrypted,ciphertext,DATA_SIZE);
...
uint8_t calcChecksum(byte *data, int len)
{
  uint8_t sum = 0;
  for(int i = 0; i < len; i++)
    sum ^= data[i];
...
void loop()
{
  if(millis()-startTime>=TEST_DURATION)
  {
    ...
    float duration = (endTime - startTime) / 1000.0; // giây
